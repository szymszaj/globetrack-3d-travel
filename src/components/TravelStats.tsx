...
export function TravelStats() {
  ...
  const stats = useMemo(() => {
    ...
    const uniqueCities = new Set(
      pins.map((pin) => `${pin.city}, ${pin.country}`)
    );
    const uniqueContinents = new Set(
      pins
        .map((pin) => CONTINENT_MAP[pin.country] || "Unknown")
        .filter((c) => c !== "Unknown")
    );
    ...
    const worldPercentage = Math.round(
      (countriesVisited / TOTAL_COUNTRIES) * 100
    );
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Out of 7 continents</p>
          <Progress value={(stats.continents / 7) * 100} className="mt-2 h-2" />
        </CardContent>
      </Card>
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Travel memories</p>
        </CardContent>
      </Card>
    </div>
  );
