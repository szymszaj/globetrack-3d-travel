  ...
  scoredCities
    ...
    .forEach((city) => {
      ...
      if (
        ...
        (uniqueCities.get(key)?.score || 0) < city.score
      ) {
...
function isStringsSimilar(
  ...
  const intersection = new Set(
    [...queryChars].filter((x) => targetChars.has(x))
  );

  const similarity =
    intersection.size / Math.max(queryChars.size, targetChars.size);
